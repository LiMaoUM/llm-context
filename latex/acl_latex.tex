...
Two findings. First, agreement is a coarse signal at deployment temperatures: with $k{=}3$, $85$--$98\%$ of cells are unanimous, so agreement takes essentially one value for most cells and its AUROC stays between $0.49$ and $0.64$, against $0.54$--$0.85$ for verbalized confidence on the same cells. Second, the two signals are nearly independent (Spearman $\rho$ between them $0.08$--$0.30$), yet combining them adds at most $0.03$ AUROC over verbalized confidence alone, and averaging verbalized confidence over the $k$ samples helps about as much ($+0.01$ to $+0.05$). Coverage-matched accuracy tells the same story: keeping the $60\%$ of full-context cells ranked highest by agreement leaves ideology accuracy essentially at its full-coverage value ($87.9\%$ vs.\ $88.2\%$ for gemma-3-27b-it), whereas ranking by verbalized confidence raises it to $96.6\%$. Agreement does carry information about item difficulty for the weaker model: qwen3-4b's full-context ideology label flips across seeds on $21.5\%$ of posts where coders split $2$--$1$ against $2.0\%$ of unanimous posts, which addresses the question of whether repeated sampling can flag items that are hard for humans. Verbalized confidence flags the same items more sharply (AUROC for coder unanimity $0.74$ vs.\ $0.60$ for agreement). % AUTHORS: figures above are from the k=3 pilot on two models; replace with the eight-model, k=10 values in Table~\ref{tab:agreement} once sample_agreement/ completes.
